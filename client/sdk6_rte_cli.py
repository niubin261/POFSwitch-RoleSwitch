...
import os, sys, argparse, pprint, collections, time, traceback, logging
...
def main():
    ...
    if os.path.exists("/home/niubin/POFSwitch-RoleSwitch/cli.log"):
        logging.basicConfig(filename = '/home/niubin/POFSwitch-RoleSwitch/cli.log',level=logging.INFO)
    ...
    def tables(args):
        if args.table_cmd == 'list':
            ...
            elif args.table_cmd == 'addj':
                logging.info("addj")
                RTEInterface.Tables.AddRuleJSON(args.tbl_id,args.file)
                print "Rule added to table %s" % (tbl_id)
            else:
    ...
    tables_parser.add_argument('table_cmd',
                               choices=('list', 'list-rules', 'add', 'edit', 'delete', 'addj'),
                               help='table command')
    ...
    tables_parser.add_argument('-j','--json',dest='file',default='',type=str,
                               help="rules in JSON format for entry commands")
