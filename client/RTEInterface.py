...
class Tables(RTEModule):
    ...
    @RPC
    def AddRuleJSON(self,tbl_id,jf):
        tbl_entry = TableEntry()
        dict_data = {}
        with open(jf,'r') as f:
            data = f.read()
            dict_data = eval(data)
            logging.info("dict_data = %s",dict_data)

            actions = str(dict_data["fwd_table"]["action"])
            match = str(dict_data["fwd_table"]["match"])
            rule_name = str(dict_data["fwd_table"]["name"])
            tbl_entry.match = match
            tbl_entry.actions = actions
            tbl_entry.rule_name = rule_name
            tbl_entry.default_rule = False
            logging.info('match = %s',tbl_entry.match)
            logging.info('rule_name = %s',tbl_entry.rule_name)
            logging.info('actions = %s',tbl_entry.actions)
        self.rte.client.table_entry_add(self.ResolveToTableId(tbl_id), tbl_entry)
    @RPC
